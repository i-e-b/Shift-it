...
namespace ShiftIt.Internal.Streaming
{
	...
	{
		[NotNull]readonly PushbackInputStream _source;
		...
		[NotNull]Stream PrepareGzip([NotNull]PushbackInputStream src)
		{
		...
		static bool LooksLikeGzip([NotNull]PushbackInputStream s)
		{
		...
		{
			return _isDone?.Invoke() ?? throw new InvalidOperationException("Compression wrapper was in an invalid state");
		}
